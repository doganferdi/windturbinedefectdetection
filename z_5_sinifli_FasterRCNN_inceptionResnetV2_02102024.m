...
numClasses = 5;
numClassesPlusBackground = numClasses;
        %katmanları değiştiriyoruz
            % lgraph = removeLayers(lgraph, {'predictions','predictions_softmax','ClassificationLayer_predictions'});
            % newLayers = [
            %          fullyConnectedLayer(numClassesPlusBackground,'Name','predictions','WeightLearnRateFactor',10,'BiasLearnRateFactor', 10)
...
save ('C:\Program Files\MATLAB\R2024b\toolbox\nnet\cnn\zeliharuzgarturbini\sonuc_5sinifli_inceptionResnetV2.mat','-v7.3');
...
ylabel("Precision")
grid on
title(["Average Precision = %.2f  boyaHasari", metrics.ClassMetrics.AP("boyaHasari")])
...
ylabel("Precision")
grid on
title(["Average Precision = %.2f  erozyon", metrics.ClassMetrics.AP("erozyon")])
...
ylabel("Precision")
grid on
title(["Average Precision = %.2f  serration", metrics.ClassMetrics.AP("serration")])
...
ylabel("Precision")
grid on
title(["Average Precision = %.2f  vortex", metrics.ClassMetrics.AP("vortex")])
...
ylabel("Precision")
grid on
title(["Average Precision = %.2f  vortexHasarli", metrics.ClassMetrics.AP("vortexHasarli")])
